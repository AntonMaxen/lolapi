...
app.get("/search/:id", async (req, res) => {
    ...
    if(!accountInfo.error) {
        res.render("accounts", {accountInfo: accountInfo});
    } else {
...
app.get("/summoner/:realm/:id/json", (req, res) => {
    let realm = req.params.realm.toLowerCase();
    let summonerName = req.params.id.toLowerCase();
    res.send(jsonProfileObj[realm][summonerName]);
});
...
app.get("/summoner/:realm/:id/matchhistory", async (req, res) => {
    ...
    if(!matchInfo.error) {
        ...
        res.render("show", {matchInfo: matchInfo, gamemode: gamemode});
        // res.send(matchInfo);
    } else {
        res.send(matchInfo);
    }
...
app.get("/summoner/:realm/:id/matchhistory/json", async (req, res) => {
    let realm = req.params.realm.toLowerCase();
    let summonerName = req.params.id.toLowerCase();
    res.send(jsonHistoryObj[realm][summonerName]);
});
